...
% proximity
% 
% Modified by Shi Qiu Sept. 18, 2019
% Probability for voting the first one
% P = (V1-V2)./# of nodes (e.g., # of nodes = 500)
% When P will be close to zero, the classification is not accurate.
% return unit: pecentage (%)

function [Y_new, cprob, votes, prediction_per_tree,proximity_ts,nodes] = classRF_predict(X,model, extra_options)
    ...
    votes_sort = sort(votes, 2, 'descend');
    
    cprob = 100.*(votes_sort(:,1) - votes_sort(:,2))./ model.ntree;
